<template>
  <view>
    ...
    <view class="address" v-if="order.delivery">
      ...
      <view class="info">
        <view class="name"
          >{{ order.delivery.username }}<text class="mobile">{{ order.delivery.mobile }}</text></view
        >
    ...
    <view class="product" v-if="order.products">
      <view
        ...
        v-for="(goodsItem, goodsIndex) in order.products"
        :key="goodsIndex"
      ...
      >
        ...
        <view class="right">
          ...
          <button
            class="action-btn"
            v-if="order.have_received != 0 && goodsItem.evaluate == false"
            @click.stop="button('evaluate', goodsItem)"
          >
            评价
          </button>
        </view>
    ...
    <view class="bottom" v-if="order.state != 9">
      ...
      <button
        class="action-btn"
        v-if="order.have_paid != 0 && order.express_number != ''"
        @click.stop="button('delivery')"
      >
        查看物流
      </button>
      <button
        class="action-btn"
        v-if="order.have_paid != 0 && order.have_received == 0"
        @click.stop="button('recerved')"
      >
        确认收货
      </button>
    ...
    async button(action, item = {}) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          break
        case 'delivery':
          this.navTo('/pages/order/express?expresssn=' + this.order.express_number + '&express=' + this.order.express)
          break
        case 'recerved':
